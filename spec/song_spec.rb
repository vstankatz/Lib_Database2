require 'rspec'
require 'song'
require 'album'
require 'pry'
require('spec_helper')

describe '#Song' do

  describe('#==') do
    it("is the same song if it has the same attributes as another song") do
      song = Song.new({:name => "Naima", :id => nil, :album_id => 1})
      song2 = Song.new({:name => "Naima", :id => nil, :album_id => 1})
      expect(song).to(eq(song2))
    end
  end
  describe('.all') do
    it("returns an empty array when there are no songs") do
      expect(Song.all).to(eq([]))
    end
  end
  describe('.all') do
    it("returns a list of all songs") do
      song = Song.new({:name => "Giant Steps", :id => nil, :album_id => 1})
      song.save()
      song2 = Song.new({:name => "Naima", :id => nil, :album_id => 1})
      song2.save()
      expect(Song.all).to(eq([song, song2]))
    end
  end

  describe('.clear') do
    it("clears all songs") do
      song = Song.new({:name => "Giant Step", :id => nil, :album_id => 1})
      song.save()
      song2 = Song.new({:name => "Naima", :id => nil, :album_id => 2})
      song2.save()
      Song.clear()
      expect(Song.all).to(eq([]))
    end
  end

  describe('#save') do
    it("saves a song") do
      song = Song.new({:name => "Naima", :id => nil, :album_id => 1})
      song.save()
      expect(Song.all).to(eq([song]))
    end
  end

  describe('.find') do
    it("finds a song by id") do
      song = Song.new({:name => "Giant Step", :id => nil, :album_id => 1})
      song.save()
      song2 = Song.new({:name => "Naima", :id => nil, :album_id => 2})
      song2.save()
      expect(Song.find(song.id)).to(eq(song))
    end
  end

  describe('.find_by_album') do
    it("finds songs for an album") do
      album = Album.new({:name => "Red", :id => nil})
      album.save
      album2 = Album.new({:name => "Blue", :id => nil})
      album2.save
      song = Song.new({:name => "Naima", :id => nil, :album_id => album.id})
      song.save()
      song2 = Song.new({:name => "California", :id => nil, :album_id => album2.id})
      song2.save()
      expect(Song.find_by_album(album2.id)).to(eq([song2]))
    end
  end

  describe('#songs') do
    it("returns an album's songs") do
      album = Album.new({:name => "Blue", :id => nil})
      album.save
      song = Song.new({:name => "Naima", :id => nil, :album_id => album.id})
      song.save()
      song2 = Song.new({:name => "California", :id => nil, :album_id => album.id})
      song2.save()
      expect(album.songs).to(eq([song, song2]))
    end
  end

  describe('#album') do
  it("finds the album a song belongs to") do
    album = Album.new({:name => "Blue", :id => nil})
    album.save
    song = Song.new({:name => "Naima", :id => nil, :album_id => album.id})
    song.save()
    expect(song.album()).to(eq(album))
  end
end

  describe('#update') do
    it("updates an song by id") do
      album = Album.new({:name => "Blue", :id => nil})
      album.save
      song = Song.new({:name => "Naima", :id => nil, :album_id => album.id})
      song.save()
      song.update("Mr. P.C.", album.id)
      expect(song.name).to(eq("Mr. P.C."))
    end
  end

  describe('#delete') do
    it("deletes an song by id") do
      album = Album.new({:name => "Blue", :id => nil})
      album.save
      song = Song.new({:name => "Giant Step", :id => nil, :album_id => album.id})
      song.save()
      song2 = Song.new({:name => "Naima", :id => nil, :album_id => album.id})
      song2.save()
      song.delete()
      expect(Song.all).to(eq([song2]))
    end
  end

end
